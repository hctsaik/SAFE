...
# ── 多參考庫嵌入審查（細粒度同類誤報：bubble vs reflection、坑洞 vs 裂縫）──
python run.py refbank --reject_from_hardneg                  # 用硬負樣本建 Reject(reflection) 庫
python run.py refbank --normal_from Workspace/<name>/train --n 200   # 自抽背景建 Normal 庫
#   接著在 config.yaml 設 audit.enabled=true（conf≥conf_high→分類；<→低信心嵌入審查/recall 撈回）
python run.py hardpos --conf 0.03            # 撈回「YOLO 信心不足但嵌入像 defect」的漏檢
#   單庫閾值分不開的細粒度誤報，改用「離 Defect/Reject/Normal 哪個近」的相對距離裁決
#   實測（坑洞 holdout）：攔截率 0.27 -> 0.45，retention 幾乎不損（見 FINDINGS.md / DESIGN §9.10）
```
...
src/
  ...
  refbank.py             # 多參考庫：建 Reject(reflection)/Normal 庫（細粒度誤報用）
  hardpos.py             # Hard-positive 撈回：低 conf 但嵌入像 defect 的漏檢
tests/smoke_test.py      # 免模型純邏輯煙測（29 項，秒級）：python tests/smoke_test.py
