...
多參考庫嵌入審查（Defect/Reflection/Normal 相對距離；細粒度誤報如 bubble vs reflection）：
  python run.py refbank --reject_from_hardneg          # 用硬負樣本建 Reject(reflection) 庫
  python run.py refbank --normal_from Workspace/<n>/train --n 200   # 自抽背景建 Normal 庫
  python run.py hardpos --conf 0.03                     # 低 conf 撈回「嵌入像 defect」的漏檢
  # 啟用審查：config.yaml 設 audit.enabled=true（需先建好 reject/normal 庫）
...
SCRIPTS = {
    ...
    "hardneg": "hardneg.py", "refbank": "refbank.py", "hardpos": "hardpos.py",
}
